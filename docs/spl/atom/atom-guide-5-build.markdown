...
resulting `.sab` (Streams Application Bundle)  executable file will be saved to the `output` folder of your project. If you choose **Build and submit job**, the `.sab` file will
...
The Console pane contains all information and error messages received from the Streaming Analytics service. Check this pane to view details of any errors that may occur during compilation.
...
You can manage and observe your running applications from the Streams Console. You can start, restart and stop applications here, view log data, and observe metrics such as throughput.
...
Let's start by looking at the output of the application. The application is simple
so whenever it detects that a bus should be receiving an alert, it just prints the alerts.  We can take a look at the output by going to the **Log Viewer**, which is opened from the far left:
...
This section will show how to provide parameters to an application right before submission.

If you have an application and would like to specify a parameter at runtime, you cannot submit the application from Atom after compilation. The application must be submitted through the Streams Console. When you submit via Console, you upload the executable `.sab` file manually.
...
It means you need to submit the application via the the Streams Console.

To get the `.sab` file downloaded,

- Compile the application using **Build**, and not **Build and submit job**
- Open the Streams Console, and upload the `.sab` file
- Set any needed parameters.


Let's continue with an example. In our case, our has a parameter called `$agency`, which is set in the code:

![param](/streamsx.documentation/images/atom/jpg/param-1.jpeg)

By default, our application has been monitoring buses in the San Francisco Municipality. We can change it to monitor buses in a different region, e.g. Toronto, by changing the `$agency` parameter right when we launch the application.
...
Submit the `.sab` file from Streams console
-------
...
Click **Browse** in the dialog that pops up, to select the SAB file from the output folder of your project.
...
In the code, this list corresponds to the list of attributes in the **param** clause:

![parameters in SPL](/streamsx.documentation/images/atom/jpg/params-in-code.jpg)
